...
// Reemplaza esto con tus claves reales
...
export default function QuinielaApp() {
  ...
  useEffect(() => {
    const fetchSupabaseData = async () => {
      try {
        ...
        setDbStatus(`🔴 Error crítico: ${error.message}`);
      }
    };
  ...
  // --- PARTIDOS PENDIENTES NO VOTADOS ---
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-4 text-white">
        ...
        <div className="bg-gray-900 p-8 rounded-xl border border-gray-800 w-full max-w-md shadow-2xl">
          <h1 className="text-3xl font-extrabold text-red-500 mb-6 text-center uppercase tracking-widest">Quiniela 2026</h1>
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans">
      <header className="bg-gray-900 p-4 border-b border-gray-800 flex justify-between items-center">
        <h1 className="text-xl font-bold text-red-500 uppercase tracking-widest">Quiniela Mundial</h1>
        <div className="flex items-center gap-4">
          <span className="text-sm text-gray-400">Analista: <strong className="text-white">{currentUser?.nombre}</strong></span>
      ...
      {/* PESTAÑAS (Modificado con colores rojos) */}
      ...
      <main className="max-w-5xl mx-auto p-4 md:p-6">
        ...
        {/* PESTAÑA: VOTAR */}
        {activeTab === 'VOTAR' && (
          <div className="bg-gray-900 p-6 rounded-xl border border-gray-800 shadow-xl max-w-xl mx-auto">
            <h2 className="text-lg font-bold text-red-500 mb-6 uppercase border-b border-gray-800 pb-2">Seleccionar Partido</h2>
            ...
            {selectedMatchId && (
              <div className="grid grid-cols-2 gap-6 bg-black p-6 rounded-lg border border-gray-800">
                <div className="text-center">
                  ...
                  <p className="font-bold text-lg mb-2 text-white">{partidos.find(p => p.id === Number(selectedMatchId))?.equipo_local}</p>
                  <input type="number" min="0" value={golesA} onChange={e => setGolesA(e.target.value)} className="w-20 text-center p-3 text-2xl font-bold rounded bg-gray-900 border border-gray-700 outline-none text-white focus:border-red-500" />
                </div>
                <div className="text-center border-l border-gray-800">
                  <label className="block text-xs text-gray-500 uppercase tracking-widest mb-2 font-bold truncate">Visitante</label>
                  <p className="font-bold text-lg mb-2 text-white">{partidos.find(p => p.id === Number(selectedMatchId))?.equipo_visitante}</p>
                  <input type="number" min="0" value={golesB} onChange={e => setGolesB(e.target.value)} className="w-20 text-center p-3 text-2xl font-bold rounded bg-gray-900 border border-gray-700 outline-none text-white focus:border-red-500" />
                </div>
              </div>
            )}
            <button onClick={handleVote} className="w-full mt-6 bg-red-600 hover:bg-red-500 text-white font-bold py-4 rounded-lg uppercase tracking-widest shadow-lg transition">Confirmar Predicción</button>
            {partidosPendientes.length === 0 && <p className="text-center text-gray-500 mt-4 text-sm">¡Ya has votado en todos los partidos pendientes!</p>}
          </div>
        )}
        ...
        {activeTab === 'MIS_VOTOS' && (
          <div className="space-y-4">
            <h2 className="text-lg font-bold text-red-500 mb-4 uppercase border-b border-gray-800 pb-2">Tu Historial de Predicciones</h2>
            {votes.filter(v => v.usuario === currentUser.usuario).length > 0 ? (
              votes.filter(v => v.usuario === currentUser.usuario).map((voto, idx) => {
                const partido = partidos.find(p => p.id === voto.partido_id);
                
                // Lógica visual para saber cuántos puntos ganó en este partido específico
                let puntosGanados = 0;
                ...
                return (
                  <div key={idx} className="bg-gray-900 p-4 rounded-lg border border-gray-800 flex justify-between items-center shadow-md relative">
                    <div>
                      <p className="text-xs text-gray-500">{partido?.date}</p>
                      <p className="font-bold text-sm md:text-lg text-white">{partido?.equipo_local} <span className="text-gray-600 mx-2 text-sm">vs</span> {partido?.equipo_visitante}</p>
                      
                      {partido?.status === 'FINISHED' && (
                        <div className={`mt-2 text-xs font-bold ${colorPuntos}`}>
        ...
        {activeTab === 'RESULTADOS' && (
          <div className="space-y-4">
            <h2 className="text-lg font-bold text-red-500 mb-4 uppercase border-b border-gray-800 pb-2">Resultados Oficiales</h2>
            {partidos.filter(p => p.status === 'FINISHED').length > 0 ? (
              partidos.filter(p => p.status === 'FINISHED').map(p => (
                <div key={p.id} className="bg-gray-900 p-4 rounded-lg border border-gray-800 flex justify-between items-center shadow-md">
                  <div>
                    <p className="text-xs text-gray-500">{p.date}</p>
                    <p className="font-bold text-sm md:text-lg text-white">{p.equipo_local} <span className="text-gray-600 mx-2 text-sm">vs</span> {p.equipo_visitante}</p>
                  </div>
                  <div className="bg-red-900/20 px-4 md:px-6 py-2 rounded-lg border border-red-500/30 text-center">
                    ...
                    <p className="text-xl md:text-2xl font-extrabold text-red-500 tracking-widest">{p.goles_local} - {p.goles_visitante}</p>
                  </div>
                </div>
              ))
            ) : (
        ...
        {activeTab === 'TABLA' && (
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            {/* Izquierda: La Tabla */}
            <div className="flex-1 bg-gray-900 rounded-xl border border-gray-800 overflow-hidden shadow-2xl">
              ...
              {calcularPuntos().map((user, idx) => (
                <div key={user.usuario} className={`p-4 flex justify-between items-center border-b border-gray-800/50 ${user.usuario === currentUser.usuario ? 'bg-red-900/10' : 'hover:bg-gray-800'}`}>
                  <span className={`w-12 text-center font-bold text-lg ${idx === 0 ? 'text-red-500' : 'text-gray-500'}`}>{idx + 1}°</span>
                  ...
                  <span className="w-20 text-center font-extrabold text-2xl text-red-500">{user.puntos}</span>
                </div>
              ))}
            </div>

            {/* Derecha: El Reglamento */}
            <div className="w-full lg:w-72 bg-gray-900 p-6 rounded-xl border border-gray-800 h-fit">
              <h3 className="text-red-500 font-bold uppercase tracking-widest border-b border-gray-800 pb-2 mb-4 text-sm">Reglamento</h3>
              <ul className="space-y-4 text-xs text-gray-400">
                <li className="flex items-start gap-2">
                  <span className="text-green-500 font-extrabold text-base leading-none">+3</span>
                  <p><strong>Resultado Exacto:</strong> Acertar la cantidad de goles de ambos equipos.</p>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-yellow-500 font-extrabold text-base leading-none">+1</span>
                  <p><strong>Acierto de Ganador:</strong> Acertar qué equipo gana o si quedan en empate, sin atinar a los goles.</p>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-red-500 font-extrabold text-base leading-none">0</span>
